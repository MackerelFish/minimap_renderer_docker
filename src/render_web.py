...
async def upload_replays_video(file: UploadFile, username: str= Depends(get_current_username)):
...
async def upload_replays_video_url(file: UploadFile, username: str= Depends(get_current_username)):
    binary_bytes = file.file.read()
    binary_stream = io.BytesIO(binary_bytes)
